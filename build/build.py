# -*- coding: utf-8 -*-
"""Body.Care site generator.
Usage:
  python3 build/build.py            Jekyll-ready output for GitHub Pages (default). Pages share
                                    _layouts/default.html; GitHub builds them on push, free plan.
  python3 build/build.py --static   Fully rendered static HTML (Netlify, Vercel, Cloudflare, S3...).
Add articles in build/content.py, tools/pages in pages_*.py, then rebuild, commit and push."""
import os, sys, json, datetime
HERE = os.path.dirname(os.path.abspath(__file__))
ROOT = os.path.dirname(HERE)
sys.path.insert(0, HERE)
import layout
layout.JEKYLL = "--static" not in sys.argv
...
if layout.JEKYLL:
    # GitHub Pages serves 404.html at any depth: inject a <base> resolving links from the site root.
    ...
    pages["404.html"] = pages["404.html"].replace("\nnopopup: true\n", "\nnopopup: true\nextra_head: " + json.dumps(fix, ensure_ascii=False) + '\nnoindex: true\npermalink: "/404.html"\n', 1)
...
if layout.JEKYLL:
    w("_layouts/default.html", layout.jekyll_layout())
    w("_config.yml", "# GitHub Pages (Jekyll) config. Pages are generated by build/build.py.\n"
      "title: Body.Care\nurl: https://body.care\nmarkdown: kramdown\n"
      "exclude: [build, README.md, PROMPTS.md, RESEARCH.md, .gitignore, .github]\n"
      "include: [.well-known]\n")
    if os.path.exists(os.path.join(ROOT, ".nojekyll")): os.remove(os.path.join(ROOT, ".nojekyll"))
else:
    w(".nojekyll", "")
